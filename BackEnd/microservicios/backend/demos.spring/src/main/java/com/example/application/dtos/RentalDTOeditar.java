...
public class RentalDTOeditar {
	...
	public static RentalDTOeditar from(Rental source) {
		return new RentalDTOeditar(source.getRentalId(), source.getRentalDate(), source.getReturnDate(),
	...
	public Rental update(Rental target) {
		...
		if (target.getInventory().getInventoryId() != inventoryId)
			target.setInventory(new Inventory(inventoryId));
		if (target.getStaff().getStaffId() != staffId)
...
//		var delAlquiler = target.getPayments().stream().filter(item -> !payments.contains(item.getPaymentId()))
//				.toList();
//		delAlquiler.forEach(item -> target.removePayment(item));
//
//		// añade pagos
//		payments.stream()
//				.filter(idPaymentDTO -> !target.getPayments().stream()
//						.anyMatch(payments -> payments.getPaymentId() == idPaymentDTO))
